...
- 买入和卖出均改为通过结算报告的局部 OCR 确认结果，并在各自流程内关闭结算页。
- 买入和卖出确认按钮均限制为单次点击；未确认结算页时安全失败，不重复盲点。
- 新增 `purchase.py` 与 `sale.py`，分别承载实际买货和卖货流程；`exchange.py` 收缩为交易所页面操作。
- `buy.py` 与 `sell.py` 暂时保留为旧导入路径的兼容转发。
- 运行日志统一为模块前缀、状态、动作和关键参数的格式。
- 增加买货、卖货与模块兼容层的回归测试。
