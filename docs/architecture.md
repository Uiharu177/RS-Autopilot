...
| 模块 | 负责内容 | 不负责内容 |
|---|---|---|
| `trade.py` | 路线初始化、回合编排、买卖顺序、城市间移动 | 交易所页面和商品页内细节 |
| `exchange.py` | 门店定位、进入交易所、买卖页签切换、离开交易所 | 商品选择、议价、买卖确认、结算页关闭 |
| `purchase.py` | 商品选择、进货书、满载保护、买入议价、买入确认、买入结算关闭 | 进入或离开交易所 |
| `sale.py` | 空车保护、出售准备、卖出议价、卖出确认、卖出结算关闭 | 进入或离开交易所 |
| `buy.py` / `sell.py` | 旧导入路径的兼容转发 | 新业务实现 |

买入和卖出均通过局部 OCR 确认各自的结算报告，并在业务流程内关闭结算页；随后由 `trade.py` 继续离开交易所或导航。
